...
exports.logoutUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        // redis.del(userId);
        // Ensure userId is a string before passing it to redis.del
        const userIdString = userId.toString(); // Convert to string if it's an ObjectId
        redis_1.redis.del(userIdString);
        res.status(200).json({
            success: true,
...
exports.updateUserInfo = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        await redis_1.redis.set(userIdString, JSON.stringify(user));
        res.status(201).json({
            success: true,
            user,
        });
