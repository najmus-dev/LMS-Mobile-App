...
export default function CourseDetailScreen() {
  ...
  // }, [user]);
  useEffect(() => {
    if (user && courseData && user.courses) {
      const hasCourse = user.courses.some((course: any) => {
        return course?.courseId?.toString() === courseData?._id?.toString();
      });
      setCheckPurchased(hasCourse);
    }
  }, [user, courseData]);
