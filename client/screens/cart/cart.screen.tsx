...
export default function CartScreen() {
  ...
  const handlePayment = async () => {
    try {
      ...
      if (paymentResponse.error) {
        ...
        // ✅ Extract paymentIntentId from client_secret
        const paymentIntentId = clientSecret.split("_secret")[0];
  
        // Prepare custom payment_info object
        const payment_info = {
          id: paymentIntentId,
          status: "succeeded", // since presentPaymentSheet didn't error
          amount,
        };
  
        // Call createOrder
        await createOrder(payment_info);
      }
    } catch (error) {
      console.error("handlePayment Error:", error);
    }
  };
  const createOrder = async (payment_info: any) => {
    const accessToken = await AsyncStorage.getItem("access_token");
    const refreshToken = await AsyncStorage.getItem("refresh_token");
  ...
  //   await axios.post(`${SERVER_URI}/create-order`,
  //       {
  //         courseId: cartItems[0]._id,
